...
public:
    ...
    CircularBuffer<T>& PushBack(T&&);
    CircularBuffer<T>& PushFront(T&&);
...
CircularBuffer<T>& CircularBuffer<T>::Realloc(const size_t& cap) {
    ...
    for (size_t i = 0; i < Size(); ++i) {
        buffer[i] = std::move(buffer_[(head_ + i) % capacity_]);
    }
    delete[] buffer_;
    buffer_ = buffer;
    tail_ = Size();
...
template <class T>
CircularBuffer<T>& CircularBuffer<T>::PushBack(T&& value) {
...
template <class T>
CircularBuffer<T>& CircularBuffer<T>::PushFront(T&& value) {
    capacity_ += (capacity_ == 0);
